...
def log_action(task, action):
    task["history"].append(f"{datetime.now().strftime('%d-%m-%Y %H:%M:%S')} - {username} ({role}) -> {action}")
...
def add_task(name, assignee):
    task = {"id": st.session_state.counter, "name": name, "status": "Pending", 
            "assigned_to": assignee, "history": [f"{datetime.now().strftime('%d-%m-%Y %H:%M:%S')} - {username} ({role}) -> Created"] }
...
for idx, task in enumerate(st.session_state.tasks):
    ...
    st.markdown(
        ...
        <div class="task-box">
            ...
            <i>Assigned to: {task['assigned_to']}</i><br>
            <details>
                <summary>History</summary>
                {'<br>'.join(task['history'])}
            </details>
        </div>
        """, unsafe_allow_html=True
    )
